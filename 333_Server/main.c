    ...
    if (ck_is_correct(client_ptr[filedes]->header, length, filedes) < 0) {
        ...
        switch (type) {
            ...
            case 14://show profile
                ...
                memcpy(buffer + HEAD_SIZE + 100, &history_record[cid].profile, sizeof(struct profile_format));

                GetPlace(history_record[cid].profile.la, history_record[cid].profile.lo, place);
                memcpy(buffer + HEAD_SIZE, &place, 80);

                //printf("%s\n", place);

                write_to_client(filedes, buffer, HEAD_SIZE + 100 + sizeof(struct profile_format));
...
void msg_to_client(int type, char client_id[20], int msg_id, char *buffer) {
    ...
    switch (type) {
        ...
        case 16: //p2p sending side notification.
            printf("notify sending side of p2p transfer");
            correct_header(16, SERVER, client_id, 0, 0, header);
            hton_header(header, buffer);
            free(header);
            break;

        case 17: //p2p receiving side notification.
            printf("notify Clientid of p2p transfer");
            memcpy(ptr, temp_list[0], 20);
            correct_header(17, SERVER, client_id, download_size, 0, header);
            hton_header(header, buffer);
            free(header);
            break;
